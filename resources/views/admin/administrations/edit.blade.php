    ...
    <div class="col-lg-12 p-0">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('administrations.update', $user->id) }}" method="post" enctype="multipart/form-data" data-parsley-validate>
                    @csrf
                    @method('PUT')
                                ...
                                @if($roles)
                                    <select name="role" class="form-control">
                                        ...
                                        @foreach($roles as $role)
                                            <option {{ old('role',(isset($user->roles[0]->id)) ? $user->roles[0]->id : '') == $role->id ? 'selected' : '' }} value="{{ $role->id }}">{{ $role->name }}</option>
                                        @endforeach
                                    </select>
                                @endif
                            </div>
