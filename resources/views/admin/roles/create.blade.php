            ...
            <div class="card-body">
                <form action="{{ route('roles.store') }}" method="post" enctype="multipart/form-data"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control ic_custom-form-input" id="ic_role_name"
                            autocomplete="off" name="name" placeholder="Enter Role Name" required
                            value="{{old('name')}}">
                        @error('name')
                    ...
                    <div class="row ic_parent_permission my-2">
                        ...
                        <div class="col-4 p-2">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" name="parent_id" class="custom-control-input"
                                    id="customCheck-{{$permission->id}}" onchange="loadChildren({{$permission->id}})">
                                <label class="custom-control-label" for="customCheck-{{$permission->id}}"></label>
                            </div>
                        </div>
                    </div>
                    <div class="row ic_div-show" id="ic_parent-{{$permission->id}}"
                        style="display: none;transition: all 10s ease">
...
$(document).ready(function() {
    $(document).on('change', '#customCheck-all', function() {
